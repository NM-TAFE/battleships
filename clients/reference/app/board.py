...
class Board:
    ...
    def check_free(self, coord, orientation, spaces):
        if (orientation not in {'H', 'V'}) and (0 > coord > 99):
            if self.playerBoard[coord] != "■" and orientation == "V":
                for i in range(1, spaces):
                    if self.playerBoard[coord + 10 * i] == "■":
                        coord = input("Space occupied, select another coordinate: ")
                        self.check_free(coord, orientation, spaces)
            elif self.playerBoard[coord] != "■" and orientation == "H":
                for i in range(1, spaces):
                    if self.playerBoard[coord + i] == "■":
                        coord = input("Space occupied, select another coordinate: ")
                        self.check_free(coord, orientation, spaces)
            ...
            self.check_free(coord, orientation, spaces)
        elif orientation != "V" and orientation != "H":
    ...
    def place_piece(self, coord, orientation, spaces):
        valid = self.check_surrounding(coord, orientation, spaces)
        valid_coord = valid[0]
        valid_orient = valid[1]
